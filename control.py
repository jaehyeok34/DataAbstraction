...
class MsgSend(BehaviorModelExecutor):
    ...
    def output(self):
        ...
        if self.recv_datas:                     # 수신 받은 data가 존재할 경우
            print("Tx routine started...")
            remote_identity = self.recv_datas[0][0] # 수신 받은 데이터의 첫번째 값은 항상 identity 값
            datas = self.recv_datas[0][1:]          # multipart 형식으로 받아오기 때문에 하나 이상의 값임
            ...
            # TODO: TERMINATED 상태에 대한 처리 필요
                ...
                print("end")
                # del self.remote_manages[remote_identity]
                # print(self.remote_manages)

            # TODO: Image 수신 루틴
            # remote에서 이미지를 전송할 때, 반드시 "send"와 같은 문자열과 image를 같은 배열로 전달해야 함
            # 즉, [b"상태값", b"image encode string..."]의 형태가 되도록
            else:           
                # datas의 0번째 index에 "상태값"이 들어가 있으므로 이를 제외한 나머지 부분이 실제 image 코드
                for data in datas[1:]:
                    save_path = f"C:\\Users\\User\\Desktop\\Programming\\cat{self.image_num}.png"   # 본인 PC에 저장하고자 하는 경로\\image파일 이름.확장자
